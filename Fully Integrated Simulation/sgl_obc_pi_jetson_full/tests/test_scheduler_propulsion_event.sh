...
PI_BIN="$1"
SRC_DIR="$2"
OUT_DIR="$SRC_DIR/build_wsl/test_out_propulsion_event"
CFG="$SRC_DIR/build_wsl/test_config_propulsion_event.json"
rm -rf "$OUT_DIR"
mkdir -p "$OUT_DIR"
cat > "$CFG" <<JSON
{
  "nominal_fraction": 0.75,
  "reserve_margin_W": 62.0,
  "lowres_N": 256,
  "highres_N": 512,
  "coarse_groups_x": 4,
  "coarse_groups_y": 4,
...
if not rows:
    ...
    raise SystemExit(3)
burn_rows=[r for r in rows if r.get('propulsion_mode','').strip('"')=='CORRECTION_BURN' and r.get('propulsion_active')=='1']
if not burn_rows:
    print('no active propulsion burn rows', file=sys.stderr)
    raise SystemExit(4)
if not any(r.get('scheduler_mode') in ('1','2') for r in burn_rows):
    print('scheduler did not throttle/suspend during propulsion burn', file=sys.stderr)
    raise SystemExit(5)
PY
